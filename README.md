...
卸载会移除插件代码、服务、Web 服务扩展配置，以及 `/www/server/webanalytics` 中的统计数据库和插件配置，但不会修改网站原始日志。重新安装时会根据服务器上仍然存在的日志自动恢复最近 30 天数据。

## 数据与隐私

安装、覆盖安装、升级、卸载重装及数据库迁移均自动完成，详见 [数据生命周期与迁移策略](docs/DATA_MIGRATION.md)。新数据库或新网站首次恢复最近 30 天；覆盖安装和升级复用已有数据库，卸载则清除插件数据。

| 数据类型 | 默认保留 | 说明 |
| --- | ---: | --- |
| 分析聚合数据 | 90 天 | PV、UV、IP、流量、趋势和维度排行 |
